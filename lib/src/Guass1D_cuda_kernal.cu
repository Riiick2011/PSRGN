...
template <typename T>
__global__ void Guass1DForward(const int nthreads, const T* bottom_data,const T* center_data,
    const T* gama_data,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    for (int it = 0; it < roi_bin_grid; it ++) // e.g., it = 0, 1
    {
      ...
      T dela = max(0.0000001,gama_u*gama_u);
      T weight= exp(-1*(x_id-center_u)*(x_id-center_u)/2.0/dela);
...
template <typename T>
__global__ void Guass1DBackwardFeature(const int nthreads, const T* top_diff,
    const T* bottom_data,
    const T* center_data,
    const T* gama_data,
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    for (int it = 0; it < roi_bin_grid; it ++) // e.g., iy = 0, 1
    {
      ...
      T df_dela = max(0.0000001,gama_u*gama_u);
      T df_dela_3 = max(0.0000001,gama_u*gama_u*gama_u);
      T df_weight= exp(-1*(df_x_id-center_u)*(df_x_id-center_u)/2.0/df_dela);
      T val = offset_bottom_data[df_t_low];

      T g = top_diff_this_bin / output_weight * df_weight;
      T g_center = top_diff_this_bin / output_weight * df_weight*(df_x_id-center_u)/df_dela*val;
      T g_gama = top_diff_this_bin / output_weight * df_weight*val*(df_x_id-center_u)*(df_x_id-center_u)/df_dela_3;
...
at::Tensor Guass_forward_cuda(const at::Tensor& input,
  ...
  AT_DISPATCH_FLOATING_TYPES(input.type(), "Guass1D_forward", [&] {
    Guass1DForward<scalar_t><<<grid, block, 0, stream>>>(
         output_size,
...
std::vector<at::Tensor> Guass_backward_cuda(const at::Tensor& grad,
  ...
  AT_DISPATCH_FLOATING_TYPES(grad.type(), "ROIAlign_backward", [&] {
    Guass1DBackwardFeature<scalar_t><<<grid, block, 0, stream>>>(
         grad.numel(),
