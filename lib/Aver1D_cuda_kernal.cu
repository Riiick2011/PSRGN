...
template <typename T>
__global__ void Aver1DForward(const int nthreads, const T* bottom_data,
...
template <typename T>
__global__ void Aver1DBackwardFeature(const int nthreads, const T* top_diff,
...
at::Tensor Average_forward_cuda(const at::Tensor& input,
  ...
  AT_DISPATCH_FLOATING_TYPES(input.type(), "Align1D_forward", [&] {
    Aver1DForward<scalar_t><<<grid, block, 0, stream>>>(
         output_size,
...
at::Tensor Average_backward_cuda(const at::Tensor& grad,
  ...
  AT_DISPATCH_FLOATING_TYPES(grad.type(), "ROIAlign_backward", [&] {
    Aver1DBackwardFeature<scalar_t><<<grid, block, 0, stream>>>(
         grad.numel(),
