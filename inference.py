...
from libs.modelings.models import PtTransformer
...
def inference(
    ...
    for idx, (vidx, fps, vlen, stride, nframes) in enumerate(
        ...
        results_per_vid = inference_single_video(
            points, fpn_masks_per_vid,
            cls_logits_per_vid, offsets_per_vid,test_cfg
            )
...
def main(args):
    ...
    if args.sigma>0:
        test_cfg["nms_sigma"]=args.sigma
    if args.iou_threshold>0:
        test_cfg["iou_threshold"]=args.iou_threshold
    model = PtTransformer(model_configs=model_cfg, train_cfg=train_cfg).cuda()
    ckpt_file = args.ckpt
    if args.topk > 0:
    ...
    for iter_idx, video_list in enumerate(val_loader,0):
        ...
        with torch.no_grad():
            ...
            fpn_masks,out_cls_logits, out_offsets,_,_ ,_,_= model([batched_input,batched_mask],False)
            points=generat_points(max_seq_len=max_len,fpn_levels=len(model.fpn_strides),
                                  scale_factor=model.scale_factor,regression_range=model.reg_range)
            output = inference(
                video_list, points, fpn_masks,
                out_cls_logits, out_offsets,test_cfg
            )
            num_vids = len(output)
