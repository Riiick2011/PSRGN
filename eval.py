...
def evaluation_proposal():
    ...
    uniform_average_nr_proposals_valid, uniform_average_recall_valid, uniform_recall_valid = run_evaluation(
        "./evaluation/thumos_gt.json",
        './outputs/detection_result.json',
        max_avg_nr_proposals=400,
    ...
    print( "AR@50 is \t",np.mean(uniform_recall_valid[:,24]))
    print( "AR@100 is \t",np.mean(uniform_recall_valid[:,49]))
    print("AR@200 is \t", np.mean(uniform_recall_valid[:,-1]))
    return np.mean(uniform_recall_valid[:,49])
