...
def postprocess_results(results, cls_score_file, num_pred=200, topk=2,flag=False):
    ...
    for vid, result in results.items():
        ...
        if not flag:
            ...
            new_pred_segment = pred_segment
            new_pred_label = pred_label

            # add to result
            processed_results['video-id'].extend([vid] * num_segs)
