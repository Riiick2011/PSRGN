...
from libs.modelings.test_layer_attention_spatial_metric import Graph_module_net_0,Graph_module_net_0_loss,Graph_module_net_0_loss_type_2,Graph_module_net_0_loss_type,Graph_module_net_0_loss_2
import numpy as np
...
class PtTransformer(nn.Module):
    ...
    def __init__(
        ...
        for l, (s, w) in enumerate(zip(self.fpn_strides, self.mha_win_size)):
            ...
            assert model_configs["max_seq_len"] % stride == 0, "max_seq_len must be divisible by fpn stride and window size"
        ...
        self.proposal_creator=ProposalCreator(parent_model=self,nms_thresh=train_cfg["iou_threshold"],n_train_pre_nms=train_cfg["pre_train_nms_topk"],
                                              n_train_post_nms=train_cfg["train_max_seg_num"],n_test_pre_nms=train_cfg["pre_test_nms_topk"],
                                              n_test_post_nms=train_cfg["test_max_seg_num"],pre_nms_thresh=train_cfg["pre_nms_thresh"],
                                              min_size=train_cfg["duration_thresh"],sigma=train_cfg["nms_sigma"],min_score=train_cfg["min_score"])
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            gt_segments=torch.ones(batched_inputs.size()[0],5,2)
            gt_masks=torch.ones(batched_inputs.size()[0],5)
        ...
        # fpn_masks: F list[B, 1, T_i] -> F List[B, T_i]
            ...
            with torch.no_grad():
                rois_i,score_i,roi_mask_i=self.proposal_creator(out_offsets=out_offsets_i,out_cls_logits=out_cls_logits_i,fpn_masks=fpn_masks_i,points=points,gt_rois=gt_segment)
        ...
        for j in range(len(fpn_feats)):
            ...
            #extend_rois[:, :, 4] = 0
            roi_feat=fpn_feats[j].unsqueeze(2)
        ...
        return fpn_masks,out_cls_logits, out_offsets,rois.cuda(),scorces.cuda(),rois_mask,cls_log
...
class PtTransformer_com(nn.Module):
    """
        Transformer based model for single stage action localization
    """
    def __init__(
    ...
    def forward(self, inputs):
        ...
        #print(cls_log.size())
            ...
            #print(new_rois[0][0],2,fpn_feats[j].size()[2])
        return fpn_masks,out_cls_logits, out_offsets,rois.cuda(),scorces.cuda(),rois_mask,cls_log,com_log

        # return loss during training



class PtTransformer_gcn(nn.Module):
    """
        Transformer based model for single stage action localization
    """
    def __init__(
        ...
        self.classify1= Graph_module_net_0(model_configs["fpn_dim"], model_configs["fpn_dim"]*2, model_configs["fpn_dim"], self.type,self.childs_num,dropout=0.7)
        self.relu=nn.ReLU()
        '''
        self.compeletness = nn.Sequential(
            MLP(model_configs["fpn_dim"]*3, model_configs["fpn_dim"] * 6, model_configs["fpn_dim"]*3, dropout=0.7),
        ...
        self.compeletness1 = Graph_module_net_0(model_configs["fpn_dim"]*3, model_configs["fpn_dim"] * 6,
                                            model_configs["fpn_dim"]*3, self.type, self.childs_num,dropout=0.7)
    @property
    def device(self):
        # a hacky way to get the device type
        # will throw an error if parameters are on different devices
    ...
    def forward(self, inputs):
        ...
        rois=torch.stack(rois,dim=0)
        rois_b,rois_n,_=rois.size()
        gcn_mask=self._sample_nodes(rois)
        scorces=torch.stack(scorces,dim=0)
        rois_mask = torch.stack(rois_mask, dim=0)
        out_cls_logits = [x.permute(0, 2, 1) for x in out_cls_logits]
        # out_offset: F List[B, 2 (xC), T_i] -> F List[B, T_i, 2 (xC)]
        ...
        cls_log1 = self.relu(self.classify1(final_pool_feats1,gcn_mask))
        ...
        com_log1 = self.relu(self.compeletness1(final_com_feats1, gcn_mask))
    ...
    def _sample_nodes(self,rois):
        ...
        ious=ious_mask*ious
        ious=ious.view(-1,num)
        low_iou_threshold=0.1
        hight_iou_threshold=0.9
        child_iou_num=10
        all_mask=[]
        targtet_thre = torch.linspace(low_iou_threshold, hight_iou_threshold, child_iou_num)
        for j in range(ious.size()[0]):
            sub_ious=ious[j]
            #print(sub_ious)
            sub_idx=self._select_idx(sub_ious,targtet_thre)
            idx_mask = F.one_hot(
                sub_idx, num_classes=num
            )
            idx_mask=idx_mask.sum(dim=0)
            all_mask.append(idx_mask)
        all_mask=torch.stack(all_mask,dim=0)
        all_mask=all_mask.view(batch,-1,num)
        return all_mask

    def _select_idx(self,source, target):
        idx_list = []
        for i in range(0, len(target)):
            m =1
            thre = target[i]
            if m==len(source):
                idx_list.append(len(source)-1)
            else:
                for j in range(m, len(source)):
                    if source[j - 1] - thre <= 0 and source[j] - thre > 0:
                        if abs(source[j - 1] - thre) <= abs(source[j] - thre):
                            idx_list.append(j - 1)
                        else:
                            idx_list.append(j)
                        break
                        ...
                        idx_list.append(j)
                        break
                    elif j==m and source[j - 1] - thre > 0:
                        idx_list.append(j - 1)
                        break
...
class PtTransformer_gcn_1(nn.Module):
    """
        Transformer based model for single stage action localization
    """
    def __init__(
        ...
        self.relu = nn.ReLU()
        self.compeletness = nn.Linear(model_configs["fpn_dim"] * 2, self.num_classes)
        self.compeletness1 = Graph_module_net_0(model_configs["fpn_dim"], model_configs["fpn_dim"] * 62,
    ...
    def forward(self, inputs):
        ...
        cls_log1 = self.relu(self.classify1(final_pool_feats1,gcn_mask,scorces_mask))
        ...
        com_log = self.compeletness(final_com_feats).view(rois_b, rois_n, -1)
        '''
        final_com_feats1 = final_pool_feats.view(rois_b, rois_n, -1)
        com_log1 = self.relu(self.compeletness1(final_com_feats1, gcn_mask,scorces_mask))
...
class PtTransformer_reg(nn.Module):
    """
        Transformer based model for single stage action localization
    """
    ...
    def __init__(
        ...
        self.classify = nn.Linear(model_configs["fpn_dim"] * 2, self.num_classes + 1)
        self.classify1 = Graph_module_net_0(model_configs["fpn_dim"], model_configs["fpn_dim"] * 2,
                                            model_configs["fpn_dim"], self.type, self.childs_num, dropout=0.7)
        self.relu = nn.ReLU()
        '''
        self.compeletness = nn.Sequential(
        ...
        self.regressor_fc = nn.Linear(model_configs["fpn_dim"] * 6, 2 * self.num_classes)
    ...
    def forward(self, inputs):
        ...
        com_log = torch.cat((final_com_feats1, com_log1), dim=2)
        com_log = com_log.view(rois_b * rois_n, -1)
        reg_log=self.regressor_fc(com_log).view(rois_b,rois_n,-1)
        com_log=self.compeletness(com_log).view(rois_b,rois_n,-1)

        # print(cls_log.size())

        # print(new_rois[0][0],2,fpn_feats[j].size()[2])
        return fpn_masks, out_cls_logits, out_offsets, rois.cuda(), scorces.cuda(), rois_mask, cls_log, com_log,reg_log
...
class PtTransformer_gcn_loss(nn.Module):
    """
        Transformer based model for single stage action localization
    """
...
'''
        ...
        self.classify = nn.Linear(model_configs["fpn_dim"] * 2, self.num_classes + 1)
        self.classify1 = Graph_module_net_0_loss(model_configs["fpn_dim"], model_configs["fpn_dim"] * 2,
                                            model_configs["fpn_dim"], self.type, self.childs_num, dropout=0.7)
        self.relu = nn.ReLU()
        '''
        self.compeletness = nn.Sequential(
        ...
        '''
        self.compeletness = nn.Linear(model_configs["fpn_dim"] * 6, self.num_classes)
        self.compeletness1 = Graph_module_net_0_loss(model_configs["fpn_dim"] * 3, model_configs["fpn_dim"] * 6,
                                                model_configs["fpn_dim"] * 3, self.type, self.childs_num, dropout=0.7)
    ...
    def forward(self, inputs,training=True):
        ...
        cls_log1,cls_gt,cls_node = self.classify1(final_pool_feats1, gcn_mask, scorces_mask,gt_pool_feats,rois)
        ...
        com_log1,com_gt,com_node = self.compeletness1(final_com_feats1, gcn_mask, scorces_mask,gt_com_feats,rois)
...
class PtTransformer_gcn_loss_type(nn.Module):
    ...
    def forward(self, inputs,training=True):
        ...
        cls_log1,cls_gt,cls_node = self.classify1(final_pool_feats1, gcn_mask1,gcn_mask2, scorces_mask,gt_pool_feats,rois)
        cls_log1=self.relu(cls_log1)
        ...
        com_log1,com_gt,com_node = self.compeletness1(final_com_feats1, gcn_mask1,gcn_mask2, scorces_mask,gt_com_feats,rois)
        com_log1=self.relu(com_log1)
