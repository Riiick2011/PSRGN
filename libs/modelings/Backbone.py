...
from libs.modelings.Block import ConvBlock,TransformerBlock,MaskedConv1D,LayerNorm,MaskedLSTM1D,LSTMBlock
from libs.utils.Functions import get_sinusoid_encoding
...
class ConvLSTMBackbone(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.ModuleList()
        for idx in range(arch[1]):
            self.stem.append(LSTMBlock(n_embd, 3, 1))

        # main branch using transformer with pooling
        self.branch = nn.ModuleList()
        for idx in range(arch[2]):
            self.branch.append(LSTMBlock(n_embd, 3, self.scale_factor))
