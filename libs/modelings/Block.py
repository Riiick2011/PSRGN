...
from libs.utils.Functions import drop_path,get_sinusoid_encoding
...
class LocalMaskedMHCA(nn.Module):
    ...
    @staticmethod
    def _new_chunk(x, window_overlap):
        """convert into overlapping chunks. Chunk size = 2w, overlap size = w"""
        # x: B x nh, T, hs
        # non-overlapping chunks of size = 2w -> B x nh, T//2w, 2w, hs
        B,T,C=x.size()
        chunk_size=(B,T//window_overlap-1,window_overlap*2,C)
        chunk_stride=(x.stride()[0],x.stride()[1]*window_overlap,*x.stride()[1:])
        # B x nh, #chunks = T//w - 1, 2w, hs
        return x.as_strided(size=chunk_size, stride=chunk_stride)
    @staticmethod
...
class MaskedLSTM1D(nn.Module):
    ...
    def __init__(
        ...
        # stride
        self.stride = stride
        self.conv = nn.LSTM(input_size=in_channels, hidden_size=out_channels, num_layers=1,
                              batch_first=True,bias=False,bidirectional=False)
    ...
    def forward(self, x, mask):
        ...
        length=new_mask.sum(dim=1).detach().cpu().int()
        out=rnn.pack_padded_sequence(x,length,batch_first=True)
        self.conv.flatten_parameters()
        out_conv,_ = self.conv(out)
        out_conv,_=rnn.pad_packed_sequence(out_conv,batch_first=True,total_length=T)
        out_conv=out_conv.permute(0,2,1)
        out_conv=self.pool(out_conv)
        # compute the mask
        if self.stride > 1:
            # downsample the mask using nearest neighbor
            out_mask = F.interpolate(
...
class LSTMBlock(nn.Module):
    ...
    def __init__(
        ...
        # 1x3 (strided) -> 1x3 (basic block in resnet)
        width = n_embd * expansion_factor
        self.conv1 = MaskedLSTM1D(
            n_embd, width, kernel_size, n_ds_stride, padding=padding)
        self.conv2 = MaskedLSTM1D(
            width, n_out, kernel_size, 1, padding=padding)

        # attach downsampling conv op
        if n_ds_stride > 1:
            # 1x1 strided conv (same as resnet)
            self.downsample = MaskedLSTM1D(n_embd, n_out, 1, n_ds_stride)
        else:
            self.downsample = None
