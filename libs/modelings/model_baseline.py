...
from libs.modelings.Backbone import ConvBackbone, ConvTransformerBackbone,ConvLSTMBackbone
...
from libs.modelings.test_layer_attention_spatial_metric import  Graph_module_net_0_loss_type
import numpy as np
from libs.datasets.Datasets import BufferList
from lib.bert_pool_old import BERT_pool_avr as Bert_pool_avr
...
class Baseline2(nn.Module):
    ...
    def forward(self, inputs, training=True):
        ...
        attention_feats=torch.cat((rois_pool_feats,final_pool_feats_1),dim=2).view(rois_b * rois_n*len(fpn_feats),-1)
        pool_weight=self.pool_weights(attention_feats).view(rois_b * rois_n, len(fpn_feats))
        pool_weight=F.softmax(pool_weight,dim=1)
        final_pool_feats=rois_pool_feats*pool_weight[:,:,None]
        final_pool_feats=torch.sum(final_pool_feats,dim=1)

        final_com_feats=com_rois_feats*pool_weight[:,:,None]
...
class Ours_model(nn.Module):
    ...
    def forward(self, inputs, training=True):
        ...
        final_pool_feats_1 = torch.bmm(rois_mask, rois_pool_feats).squeeze(1)
        final_pool_feats_1=final_pool_feats_1.unsqueeze(1).repeat(1,len(fpn_feats),1)
        attention_feat=torch.cat((rois_pool_feats,final_pool_feats_1),dim=2).view(rois_b * rois_n* len(fpn_feats), -1)
        ...
        scorces_mask = (scorces > 0.6).float()
...
class Ours_model_2(nn.Module):
    ...
    def forward(self, inputs, training=True):
        ...
        scorces_mask = (scorces > 0.0).float()
        final_com_feats=com_rois_feats*pool_weights[:,:,None]
        final_com_feats=torch.sum(final_com_feats,dim=1)
        # cls_log = self.classify(final_pool_feats).view(rois_b, rois_n, -1)
...
class Ours_model_avg(nn.Module):
    """
        Transformer based model for single stage action localization
    """
    ...
    def __init__(
        ...
        self.pool = Bert_pool_avr(input_dim=model_configs["fpn_dim"])
        self.proposal_creator = ProposalCreator(parent_model=self, nms_thresh=train_cfg["iou_threshold"],
                                                n_train_pre_nms=train_cfg["pre_train_nms_topk"],
                                                n_train_post_nms=train_cfg["train_max_seg_num"],
                                                n_test_pre_nms=train_cfg["pre_test_nms_topk"],
    ...
    def forward(self, inputs, training=True):
        ...
        for i in range(batch):
            ...
            scorces.append(score_i)
            rois_mask.append(roi_mask_i)
        rois = torch.stack(rois, dim=0).cuda()
        rois_b, rois_n, _ = rois.size()
        #print(rois_n)
        gcn_mask1, gcn_mask2 = self._sample_nodes(rois)
        scorces = torch.stack(scorces, dim=0)
        ...
        for j in range(len(fpn_feats)):
            new_rois = rois / self.fpn_strides[j]
            extend_rois = new_rois.new_zeros(new_rois.size()[0], new_rois.size()[1], 3)
            extend_rois[:, :, 0] = new_rois[:, :, 0]
            extend_rois[:, :, 1] = new_rois[:, :, 1]
            # extend_rois[:, :, 2] = 0
            extend_rois[:, :, 2] = new_rois[:, :, 2]
            # extend_rois[:, :, 4] = 0
            roi_feat = (fpn_feats[j] * fpn_masks[j][:, None, :])
            mask_sum=fpn_masks[j].sum(dim=1)
            mask_sum=mask_sum.clamp(min=1.0)
            global_feat=roi_feat.sum(dim=2)
            global_feat=global_feat/mask_sum[:,None]

            left_extend_rois = extend_rois.new_zeros(new_rois.size()[0], new_rois.size()[1], 3)
            right_extend_rois = extend_rois.new_zeros(new_rois.size()[0], new_rois.size()[1], 3)
            left_extend_rois[:, :, 0] = extend_rois[:, :, 0]
            left_extend_rois[:, :, 1] = extend_rois[:, :, 1] - (extend_rois[:, :, 2] - extend_rois[:, :, 1]) * 0.5
            left_extend_rois[:, :, 2] = extend_rois[:, :, 1]
            left_extend_rois[:, :, 1] = left_extend_rois[:, :, 1].clamp(min=0.0, max=self.max_seq_len)
            right_extend_rois[:, :, 0] = extend_rois[:, :, 0]
            right_extend_rois[:, :, 1] = extend_rois[:, :, 2]
            right_extend_rois[:, :, 2] = extend_rois[:, :, 2] + (extend_rois[:, :, 2] - extend_rois[:, :, 1]) * 0.5
            right_extend_rois[:, :, 1] = right_extend_rois[:, :, 1].clamp(min=0.0, max=self.max_seq_len)
            extend_rois = extend_rois.view(-1, 3)
            left_extend_rois = left_extend_rois.view(-1, 3)
            right_extend_rois = right_extend_rois.view(-1, 3)
            sub_pool_feats = self.pool(roi_feat, global_feat,extend_rois)
            left_pool_feats = self.pool(roi_feat, global_feat,left_extend_rois)
            left_pool_feats = left_pool_feats.view(rois_b, rois_n, -1)
            right_pool_feats = self.pool(roi_feat,global_feat, right_extend_rois)
            right_pool_feats = right_pool_feats.view(rois_b, rois_n, -1)
            sub_pool_feats = sub_pool_feats.view(rois_b, rois_n, -1)
            com_pool_feats = torch.cat((left_pool_feats, sub_pool_feats, right_pool_feats), dim=2)
            rois_pool_feats.append(sub_pool_feats)
        ...
        scorces_mask = (scorces > 0.5).float().cuda()
        final_com_feats=com_rois_feats*pool_weights[:,:,None]
        final_com_feats=torch.sum(final_com_feats,dim=1)
        # cls_log = self.classify(final_pool_feats).view(rois_b, rois_n, -1)
...
class Baseline12(nn.Module):
    ...
    def __init__(
        ...
        self.backbone = ConvLSTMBackbone(n_in=model_configs["input_dim"], n_embd=model_configs["embd_dim"],
                                     n_embd_ks=model_configs["embd_kernel_size"],
                                     arch=model_configs["backbone_arch"],
                                     scale_factor=model_configs["scale_factor"],
                                     with_ln=model_configs["embd_with_ln"])
    ...
    def forward(self, inputs, training=True):
        ...
        final_pool_feats_1 = torch.bmm(rois_mask, rois_pool_feats).squeeze(1)
        final_pool_feats_1 = final_pool_feats_1.unsqueeze(1).repeat(1, len(fpn_feats), 1)
        attention_feats = torch.cat((rois_pool_feats, final_pool_feats_1), dim=2).view(rois_b * rois_n * len(fpn_feats),
                                                                                       -1)
        pool_weight = self.pool_weights(attention_feats).view(rois_b * rois_n, len(fpn_feats))
        pool_weight = F.softmax(pool_weight, dim=1)
        final_pool_feats = rois_pool_feats * pool_weight[:, :, None]
    ...
    def __init__(
        ...
        if model_configs["backbone_type"] == 'convTransformer':
            self.backbone = ConvTransformerBackbone(
                ...
                max_len=model_configs["max_seq_len"],
                arch=model_configs["backbone_arch"],
                mha_win_size=[-1]*6,#self.mha_win_size,
                scale_factor=model_configs["scale_factor"],
                with_ln=model_configs["embd_with_ln"],
                attn_pdrop=0.0,
                proj_pdrop=self.train_dropout,
