...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key,roi_mask,node_num, mask=None):
...
class MultiHeadedAttention_2_min(nn.Module):
    """
    Take in model size and number of heads.
    """
...
class MultiHeadedAttention_2_type(nn.Module):
    """
    Take in model size and number of heads.
    """
    ...
    def forward(self, query, key, roi_mask1,roi_mask2,node_num,mask=None):
        batch_size,num,_,chinnal = query.size()
...
class MultiHeadedAttention_2_type_min(nn.Module):
    """
    Take in model size and number of heads.
    """
    ...
    def forward(self, query, key, roi_mask1,roi_mask2,node_num,mask=None):
        ...
        atten_mask[:,:,idces1,:]=1.0
        attn1_min=F.relu(attn1-one_mm[None,:,:,None])*-1
...
class Graph_module_net_0_loss(nn.Module):
    ...
    def __init__(self, in_features,mid_features, out_features,type_num,childs_num,multi_head=1,dropout=0.1, atten_dropout=0.1,group=4,bias=True):
        ...
        self.childs_num=childs_num
        self.dropout = dropout
        self.atten=MultiHeadedAttention_2_min(h=self.multi_head,d_model=in_features,dropout=atten_dropout)
        self.agg1 = nn.Sequential(nn.Conv1d(in_channels=in_features, out_channels=mid_features,
                                            kernel_size= 1, stride=1, groups=group,
                                            bias=bias),
                                  nn.ReLU(inplace=True))
...
class Graph_module_net_0_loss_type(nn.Module):
    ...
    def __init__(self, in_features,mid_features, out_features,type_num,childs_num,multi_head=1,dropout=0.1, atten_dropout=0.1,group=4,bias=True):
        ...
        self.childs_num=childs_num
        self.dropout = dropout
        self.atten=MultiHeadedAttention_2_type_min(h=self.multi_head,d_model=in_features,dropout=atten_dropout)
        self.agg1 = nn.Sequential(nn.Conv1d(in_channels=in_features, out_channels=mid_features,
                                            kernel_size= 1, stride=1, groups=group,
                                            bias=bias),
                                  nn.ReLU(inplace=True))
...
class Graph_module_net_0_loss_type_2(nn.Module):
    ...
    def __init__(self, in_features,mid_features, out_features,type_num,childs_num,multi_head=1,dropout=0.1, atten_dropout=0.1,group=4,bias=True):
        ...
        self.dropout = dropout
        self.atten=MultiHeadedAttention_2_type_min(h=self.multi_head,d_model=in_features,dropout=atten_dropout)
        self.atten2 = MultiHeadedAttention_2_type(h=self.multi_head, d_model=mid_features, dropout=atten_dropout)
        self.agg1 = nn.Sequential(nn.Conv1d(in_channels=in_features, out_channels=mid_features,
                                            kernel_size= 1, stride=1, groups=group,
                                            bias=bias),
                                  nn.ReLU(inplace=True))
