...
from libs.modelings.models import PtTransformer_com,PtTransformer_reg
...
def inference(
        video_list,
        rois,
        scores, cls_scores,com_scores,reg_log,
        test_cfg,regssion=False
    ):
    results = []
    ...
    for idx, (vidx, fps, vlen, stride, nframes) in enumerate(
        ...
        reg_log_vid = reg_log[idx].contiguous()

        reg_log_vid =reg_log_vid.view(-1,2)
        ...
        final_reg_per_vid=reg_log_vid[keep_idxs1]
        ...
        final_reg_per_vid=final_reg_per_vid[idxs]
        final_scores_per_vid = final_scores_per_vid[:num_topk].clone()
        final_reg_per_vid=final_reg_per_vid[:num_topk].clone()
        ...
        if regssion:
            ...
            new_center = center + duration * final_reg_per_vid[:, 0]
            new_duration = duration * torch.exp(final_reg_per_vid[:, 1])
            new_detections = torch.cat((
                torch.clamp(new_center - new_duration / 2.0,min= 0,)[:, None],
                torch.clamp(new_center + new_duration / 2.0, min=0)[:, None],
            ), dim=1)
            rois_per_vid=new_detections
...
def main(args):
    ...
    model = PtTransformer_reg(model_configs=model_cfg, train_cfg=train_cfg).cuda()
    ckpt_file = args.ckpt
    if args.topk > 0:
    ...
    for iter_idx, video_list in enumerate(val_loader,0):
        ...
        with torch.no_grad():
            ...
            fpn_masks,out_cls_logits, out_offsets,rois,scorces,rois_mask,cls_log,com_log,reg_log= model([batched_input,batched_mask])
            #print(com_log)
            regression=False
            cls_log=F.softmax(cls_log,dim=2)
                ...
                #print(cls_log[0][0])
            output = inference(
                video_list, rois,scorces,cls_log,com_log,reg_log,test_cfg,regression
            )
            num_vids = len(output)
    ...
    if det_eval is not None:
        ...
        # call the evaluator
        _, mAP = det_eval.evaluate(results, verbose=True)
    else:
