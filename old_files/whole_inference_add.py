...
cmd0='python fastai_train_add.py ./configs/thumos_i3d.yaml'
os.system(cmd0)
...
for j in range(30,61):
    sigma=j/100.0
    print(j)
    cmd = 'python com_inference_add.py ./configs/thumos_i3d.yaml ./models/epoch_%d_model_GCN.pth'%j#+' --sigma '+str(sigma)
    os.system(cmd)
...
for i in new_flie:
    for s in range(11,35):
        sigma=s/100.0
        print(sigma,i)
        cmd1 = 'python com_inference_add.py ./configs/thumos_i3d.yaml ./models/epoch_%d_model_GCN.pth'%i+' --sigma '+str(sigma)
        os.system(cmd1)
...
new_name='epoch_%d_%.4f_%.2f_model_GCN_add.pth'%(int(ap1[-1][2]),float(ap1[-1][0]),float(ap1[-1][1]))
